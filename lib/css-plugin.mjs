  ...
  return {
    ...
    async load(id) {
      ...
      if (id === prerenderCSSModule) {
        // This is a horrible hack, but gotta ship soon so…
        const prerenderResults = [...pathToResult]
          .filter(
            ([path]) =>
              path.includes('/src/static-build/') ||
              path.includes('/src/shared/'),
          )
          .map(([_, result]) => result.css);

        return `export default ${JSON.stringify(
          prerenderResults.join(''),
        ).replace(
          assetRe,
          (match, hash) =>
